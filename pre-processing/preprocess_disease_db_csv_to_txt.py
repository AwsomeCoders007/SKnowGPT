...
df_database = pd.read_csv("../data/DisTreatKG/DisTreatKG_disease_dataset.csv") 

for index, row in df_database.iterrows():
    with open("../data/DisTreatKG/DisTreatKG_train_test.txt","a") as f:   
        ...
        for medical_test in row["TestsAndProcedures"].strip('[]').split(","):
            f.write(row["disease"].replace(" ","_"))
            f.write("\t")
        ...
        for medication in row["commonMedications"].strip('[]').split(","):
            f.write(row["disease"].replace(" ","_"))
            f.write("\t")
